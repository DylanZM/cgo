  ...
  return (
    <div className="h-full flex flex-col bg-[var(--surface)] border-l border-[var(--border)] w-72">
      <div className="flex items-center justify-between px-4 h-10 border-b border-[var(--border)] shrink-0">
        <span className="text-xs font-medium text-[var(--text)]">Version History</span>
        ...
        {history.length === 0 ? (
          ...
          <div className="p-2 space-y-1">
            {history.map((entry, i) => (
              <div
                key={entry.id}
                className="group relative p-3 rounded-lg border border-transparent hover:border-[var(--border)] hover:bg-[var(--surface-2)] transition-colors"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    ...
                    {entry.error && (
                      <span className="inline-block mt-1 px-1.5 py-0.5 text-[9px] rounded bg-[var(--error-dim)] text-[var(--error)]">
                        Error
                      </span>
                    )}
                    {!entry.error && entry.output && (
                      <span className="inline-block mt-1 px-1.5 py-0.5 text-[9px] rounded bg-[var(--success-dim)] text-[var(--success)]">
                        OK
                      </span>
                    )}
                  </div>
      ...
      {history.length > 0 && (
        <div className="p-3 border-t border-[var(--border)]">
          <button
            onClick={onClear}
            className="w-full px-3 py-1.5 text-xs text-[var(--error)] hover:bg-[var(--error-dim)] rounded-md transition-colors"
          >
            Clear history
          </button>
        </div>
